# Hi, I'm Kevin Campos 👋

## Full Stack Software Engineer

Building Enterprise Software, SaaS Platforms, APIs, AI Automation and Cloud Solutions.

📍 Cartago, Costa Rica
🌐 Portfolio: https://kevilyncamposrodriguez.net
💼 LinkedIn: [linkedin.com/in/kevilyncamposrodriguez](https://www.linkedin.com/in/kevilyncamposrodriguez/)

---

## About Me

Senior Full Stack Software Engineer with 8+ years of experience developing enterprise software, SaaS platforms, APIs, cloud infrastructure, and mission-critical integrations for companies in Costa Rica and the United States.

I specialize in designing and delivering scalable business solutions from concept to production, including architecture, backend development, frontend applications, infrastructure, deployment, and automation.

---

## Core Technologies

### Backend

- Python
- Django
- Django REST Framework
- Laravel
- PHP
- Node.js

### Frontend

- React
- TypeScript
- Vite
- Material UI
- Livewire

### Databases

- PostgreSQL
- MySQL
- Redis

### Infrastructure

- Docker
- Linux
- Nginx
- Cloudflare
- CloudPanel
- VPS Administration
- SSL & DNS Management

### Artificial Intelligence

- OpenAI
- Google Gemini
- PDF Processing
- Structured Data Extraction
- AI Automation

---

## Enterprise Integrations

- QuickBooks Online
- QuickBooks Desktop SDK
- Salesforce CRM
- Costa Rica Tax Authority (Electronic Invoicing)
- Retail Link Walmart
- Stripe
- MercadoPago
- INS APIs
- WhatsApp & Meta Integrations

---

## Featured Projects

### Prisma CMA

AI-powered multi-insurance comparison platform.

**Stack:**
Django • React • PostgreSQL • Redis • Celery • OpenAI • Gemini

---

### Prisma Storep

Insurance platform ecosystem including API Gateway, Admin Panel and Web Store.

**Stack:**
Django • React • REST APIs • PostgreSQL

---

### FacturaRapida

Electronic invoicing platform active since 2017.

**Stack:**
Laravel • MySQL • Electronic Invoicing CR

---

### ControlPro Solutions

Route sales management platform with offline synchronization.

**Stack:**
Laravel • JavaScript • MySQL

---

### Retail API Walmart

Inventory, orders and reporting integration platform.

**Stack:**
Django • DRF • PostgreSQL • React

---

## Professional Experience

- Prisma Insurance Broker
- ARMEL CR Distributor
- Otecno S.A.
- Contafast S.A.
- ITMS Group
- ScheduleInterpreter.com
- Samsatec S.A.

---

## Highlights

✔ 8+ Years of Software Development Experience

✔ 20+ Enterprise Projects Delivered

✔ 10+ Third-Party System Integrations

✔ Founder of Otecno S.A.

✔ SaaS Architectures, APIs & AI Automation

✔ Cloud Infrastructure & VPS Administration

---

## Current Focus

- Enterprise Software
- SaaS Platforms
- AI Automation
- Django & React
- QuickBooks Integrations
- Cloud Infrastructure

---
## Featured Projects

Some of the projects presented in my portfolio were developed for clients or companies and are subject to confidentiality restrictions.

While the source code cannot be publicly shared, I provide:

- Screenshots
- Architecture diagrams
- Technical case studies
- Technology stack details
- Key achievements and outcomes


---
## Contact

📧 k.camposr05@outlook.com

🌐 https://kevilyncamposrodriguez.net

💼 LinkedIn: [linkedin.com/in/kevilyncamposrodriguez](https://www.linkedin.com/in/kevilyncamposrodriguez/)
